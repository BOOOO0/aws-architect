...
- AWS가 제공하는 파일 시스템이며 NFS 프로토콜(TCP/2049)을 사용한다. 윈도우용인 FSx는 SMB 프로토콜을 사용한다.

- NFS처럼 마운트해서 사용할 수 있으며 완전관리형 서비스로 서버를 따로 생성해서 마운트하지 않고 EFS만 생성해서 필요한 서버에서 마운트하면 된다.

- 고가용성, 자동 조정, 자동 백업 기능을 제공한다. 자동 조정은 더 큰 파일 시스템이 필요할 때 별도로 확장할 필요 없이 늘어나는 것 같다. 근데 그래도 확장이 되면 그걸 마운트해서 쓰는 서버에서는 반영하기 위해서 작업이 필요한거 아닌가???

- EFS도 완전관리형이지만 NFS VM을 쓰는 것과 마찬가지로 하나의 서버에서 공유 파일 시스템을 제공하는 것이기 때문에 보안 그룹을 적용할 수 있다.

- **EFS는 VPC 내에 존재한다.**

## EFS 생성

- 하나의 가용 영역에서만 마운트 가능하도록 하는 One Zone 유형과 리전 유형 중 하나를 선택할 수 있다. 비용은 리전이 더 비싸다.

- 데이터에 대한 접근의 주기가 얼만큼 되는 지에 따라 IA, Archive로 전환되도록 설정할 수 있다.

- 탑재 대상은 가용 영역당 하나의 서브넷에 탑재할 수 있다.

- 그 다음 파일 시스템 정책을 정할 수 있다.

- 생성한 EFS의 세부 정보에서 연결을 누르면 연결 방법에 대한 가이드가 나온다. DNS, IP를 통해 탑재할 수 있다.

- 탑재 방법은 마운트 명령어로 EFS로도 가능하고 NFS로도 가능하다.

- `sudo mount -t efs -o tls [efs ID]:/ [마운트 목적지 디렉토리명]`

- `sudo mount -t nfs4 -o nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport [efs ID].efs.ap-northeast-2.amazonaws.com:/ [마운트 목적지 디렉토리명]`

- CentOS는 NFS와 관련된 내용이 기본적으로 있어서 바로 되지만 Ubuntu에는 없어서 nfs 타입으로 명령어를 치면 bad option 에러가 발생한다.

- nfs-common 패키지를 설치하면 정상적으로 마운트가 된다.

- CentOS와 같은 계열의 OS에서 NFS가 되지 않을 때는 nfs-utils 패키지에 클라이언트가 포함되어 있으니 설치하면 된다.

- 양쪽에 마운트가 되면 마운트된 EFS 파일 시스템을 통해 파일 공유가 가능하다.

- EFS에 한 가용 영역에 있는 인스턴스에서 저장을 하면 다른 모든 가용 영역의 EFS에 그 저장 상태가 반영이 된다.

# FSx

- 4가지 FSx 종류 중 하나를 선택해서 생성할 수 있다.

- 각 종류마다 사용하는 프로토콜이 다르며 FSx for Win File Server가 SMB 프로토콜을 사용하는 파일 시스템 서버이다.

# S3 (Simple Storage Service)

- 이거 설명하고 생성 시작할 때 학적과에서 전화와서 전공선택 3학점 모자르다고 제대로 확인 안하고 전화와서 ㄹㅇ 돌뻔해서 정리 못했다. 이부분은 그래도 저번 교육때 자세한 정리가 있으니까 그걸 보자 진짜 힘들었다..... 자기가 잘못 알고서 왜....

## 정적 웹사이트 호스팅

- S3에 정적 웹사이트를 배포할 수 있다. 엔트리 포인트를 지정해주고 WS의 홈 디렉토리에 배포하듯이 똑같이 구성하면 된다.

### 퍼블릭 액세스 차단 편집

- S3 버킷의 퍼블릭 엑세스를 설정할 수 있다.
